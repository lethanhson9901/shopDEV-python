...
# # Configure logging (initialize the logger once)
# if not hasattr(DBManager, 'logger'):
#     DBManager.logger = setup_logger()

class DBManager:
    def __init__(self, db: Optional[Database] = None):
        self._db_instance = db or Database()
        self._db = None  # Will be set when get_db is first called
        self.logger = setup_logger()  # Use the same logger configuration
    ...
    async def get_db(self):
        if self._db is None:
            try:
                ...
                DBManager.logger.error(f"Error establishing a database connection: {e}")
                raise e
        return self._db
    ...
    async def find_user_by_email(self, email: str) -> Optional[dict]:
        try:
            ...
            DBManager.logger.error(f"Error finding a user by email: {e}")
            raise e
    ...
    async def insert_user(self, user_data: dict) -> InsertOneResult:
        try:
            ...
            DBManager.logger.error(f"Error inserting a user: {e}")
            raise e
    ...
    async def delete_user_by_email(self, email: str) -> DeleteResult:
        try:
            ...
            DBManager.logger.error(f"Error deleting a user by email: {e}")
            raise e
    ...
    async def update_user(self, email: str, update_data: dict) -> UpdateResult:
        try:
            ...
            DBManager.logger.error(f"Error updating a user: {e}")
            raise e
